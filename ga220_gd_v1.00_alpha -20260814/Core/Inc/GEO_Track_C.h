...
extern OrientLoadTypeDef OrientLoad;//平台自身指向角，°
...
//---------------平台自身角度至空间指向角转换计算---------------------//
OrientTypeDef Orient_Convert(Triangle_Fun_Cal_TypeDef Triangle_Fun_Cal_Result);
//仅针对使用机载/车载惯导的情况，如果有内置惯导，空间指向角由惯导直接给出；
//方位、俯仰角（两轴四框架为综合角度）转换至光轴的空间方位、俯仰指向角;
//对于横置吊舱，首先由伺服程序将俯仰、横滚角度转换为等效纵置转塔的方位、俯仰角度（两轴四框架为综合角度），再转换为光轴的空间方位、俯仰指向角;
//----------------理解错误，无效--------------------------------------------//
...
//---------------------20250627，思路错误，仍然解算为方位、俯仰空间指向角,再往吊舱俯仰、横滚框架转换------------//
//---------该函数不需要--------------------//
