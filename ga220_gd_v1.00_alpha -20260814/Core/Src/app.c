...
/** RTK NMEA 主循环处理窗口（ms），TIM6 计满后锁定 */
#define APP_RTK_NMEA_RUN_MS  (120000U)
static uint32_t s_rtk_run_ms_cnt;
static uint8_t  s_rtk_nmea_locked;
...
/** 本机/目标地理坐标及视线角输出 */
GeoPoint_t lxy_self, lxy_target , gps_self , gps_target;
...
{
#if 0
    if(GeoLos_CalcAbsAngle(&lxy_self, &lxy_target, &lxy_out) != GEO_LOS_OK)
	{
		lxy_out.pitch_deg = 0;
		lxy_out.yaw_deg = 0;
	}
#endif
#if 1
	Target.lattitude = deg__(lxy_target.lat_deg);
	Target.longitude = deg__(lxy_target.lon_deg);
	Target.height = lxy_target.alt_m;
	INS.lattitude = deg__(lxy_self.lat_deg);
	INS.longitude = deg__(lxy_self.lon_deg);
	INS.height = lxy_self.alt_m;
	PT_Angle.Fw = FWControl.P_fb;
...
static void APP_RtkNmea_Apply(void)
{
    ...
    static uint8_t  s_snap_init = 0U;
    static double   s_gga_lat;
    static double   s_gga_lon;
    static float    s_gga_alt;
    static float    s_tra_heading;
    static float    s_hdt_heading;
    static uint8_t  s_tra_valid;
    static uint8_t  s_hdt_valid;
    static float    s_yaw_filt = 0.f;
    static uint8_t  s_yaw_init = 0U;
    uint8_t pos_dirty = 0U;
    uint8_t yaw_dirty = 0U;
    float lat;
    float lon;
    float alt;
    float yaw_raw = 0.f;
    ...
    if (s_snap_init == 0U)
    {
        s_gga_lat       = rtk->gga.lat_deg;
        s_gga_lon       = rtk->gga.lon_deg;
        s_gga_alt       = rtk->gga.alt_m;
        s_tra_heading   = rtk->tra.heading_deg;
        s_hdt_heading   = rtk->hdt.heading_deg;
        s_tra_valid     = rtk->tra.valid;
        s_hdt_valid     = rtk->hdt.valid;
        s_snap_init     = 1U;
        return;
    }

    /*
     * 分类型脏标志：仅 gga/tra/hdt 字段变化时写入，RMC 等只更新
     * g_rtk_nmea_data.rmc 时不会误触位置或航向路径。
     */
    if ((rtk->gga.valid != 0U) &&
        (rtk->gga.qual == (uint8_t)RTK_NMEA_QUAL_SPP))
    {
        if ((rtk->gga.lat_deg != s_gga_lat) ||
            (rtk->gga.lon_deg != s_gga_lon) ||
            (rtk->gga.alt_m != s_gga_alt))
        {
            pos_dirty = 1U;
        }
    }
    ...
    if (rtk->tra.valid != 0U)
    {
        if ((rtk->tra.valid != s_tra_valid) ||
            (rtk->tra.heading_deg != s_tra_heading))
        {
            yaw_dirty = 1U;
            yaw_raw   = rtk->tra.heading_deg;
        }
    }
    else if (rtk->hdt.valid != 0U)
    {
        if ((rtk->hdt.valid != s_hdt_valid) ||
            (rtk->hdt.heading_deg != s_hdt_heading))
        {
            yaw_dirty = 1U;
            yaw_raw   = rtk->hdt.heading_deg;
        }
    }
    else if ((rtk->tra.valid != s_tra_valid) || (rtk->hdt.valid != s_hdt_valid))
    {
        s_tra_valid   = rtk->tra.valid;
        s_hdt_valid   = rtk->hdt.valid;
    }

    if ((pos_dirty == 0U) && (yaw_dirty == 0U))
    {
        return;
    }
    ...
    if (pos_dirty != 0U)
    {
        lat = (float)rtk->gga.lat_deg;
        lon = (float)rtk->gga.lon_deg;
        alt = rtk->gga.alt_m;

        gps_self.lat_deg = lat;
        gps_self.lon_deg = lon;
        gps_self.alt_m   = alt;

//        INS.height    = (double)alt;
...
//        INS.Roll  = rtk->tra.roll_deg;
    ...
    /* 0.05° 死区 + 一阶低通，抑制 RTK 航向噪声引起的伺服抖动 */
    if ((dyaw > -0.05f) && (dyaw < 0.05f))
    {
        return;
    }

    s_yaw_filt += 0.25f * dyaw;
    if (s_yaw_filt >= 360.f)
    {
        s_yaw_filt -= 360.f;
    }
    else if (s_yaw_filt < 0.f)
    {
        s_yaw_filt += 360.f;
    }
    yaw_ing = s_yaw_filt;
	
	send_image_cmd_buffer.send_cmd_struct.self_fwangle = (uint16_t)((yaw_ing + 90)*100);  
...
static void CMD_REC_Revise(uint8_t *data)
{
    ...
    roll    = (float)roll16 ;
    fw_comp = (int32_t)(((data[15] << 24) | (data[14] << 16) | (data[13] << 8) | data[12]));
    gd_comp = (int32_t)(((data[19] << 24) | (data[18] << 16) | (data[17] << 8) | data[16]));
	...
	lxy_self.alt_m = alt;
	lxy_self.lat_deg = (float)lat / 100000;
	lxy_self.lon_deg = (float)lon / 100000;
...
//    if (FWControl.P_fb < 0) {
//        /* 负反馈角先转到 [0,360) 再加补偿 */
//		fw_angle_middle = (FWControl.P_fb + 360) * 100;
////        fw_angle_middle += INS.Yaw ;
//		if (fw_angle_middle > 36000) {
//            fw_angle_middle -= 36000;
//        }
//        
//    } else {
//        fw_angle_middle = (FWControl.P_fb + 0) * 100;
////        fw_angle_middle += INS.Yaw;
//		if (fw_angle_middle > 36000) {
//            fw_angle_middle -= 36000;
//        }
//    }
	...
	send_image_cmd_buffer.send_cmd_struct.fw_angle = (uint16_t)(Orient.Orient_Yaw * 100);//(uint16_t)fw_angle_middle;
    /* 俯仰角/角速度直接取 GD 轴反馈 */
    send_image_cmd_buffer.send_cmd_struct.fy_angle = (int16_t)(Orient.Orient_Pitch * 100);//(int16_t)(GDControl.P_fb * 100) + (int16_t)INS.Pitch ;
    ...
    send_image_cmd_buffer.send_cmd_struct.len  = 74;
	send_image_cmd_buffer.send_cmd_struct.self_longitude =(int)(gps_self.lon_deg * 100000);  
	send_image_cmd_buffer.send_cmd_struct.self_latitude = (int)(gps_self.lat_deg * 100000);  
	send_image_cmd_buffer.send_cmd_struct.self_high =    (int)gps_self.alt_m;      
	
	send_image_cmd_buffer.send_cmd_struct.self_fyangle = (int16_t)(INS.Pitch*100);  
	send_image_cmd_buffer.send_cmd_struct.self_hgangle = (int16_t)(INS.Roll*100);  
...
static void USER_Ctrl_OptCalibFlashApply(void)
{
    ...
    fw_comp       = (int32_t)s_opt_calib_flash.fw_comp;
    INS.Roll      = (float)s_opt_calib_flash.roll;
    gd_comp     = (int32_t)s_opt_calib_flash.gd_comp;
}
...
void APP_Ctrl_System_Handle(void)
{
    ...
    /* s_rtk_nmea_locked==0 时解析 RTK；TIM6 计满 2min 后置位锁定 */

    if (s_rtk_nmea_locked == 0U)
    {
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
    if (htim == &htim6) {
        ...
        {
            s_rtk_run_ms_cnt++;
            if (s_rtk_run_ms_cnt >= APP_RTK_NMEA_RUN_MS)
            {
                s_rtk_nmea_locked = 1U;
            }
        }
    }
}
