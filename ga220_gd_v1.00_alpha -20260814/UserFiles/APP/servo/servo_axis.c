...
/* TODO: 根据实际编码器分辨率调整 */
#define SF_BMQ_COF_FW 0.0000214576721f /**< FW 每1个码对应的角度 */
#define SF_BMQ_COF_GD 0.0000214576721f /**< GD 每1个码对应的角度 */
// 21bit:0.0001716613769f; 20bit:0.0003433227539f; 24bit:0.0000214576721f
...
void dev_servo_axis_init(dev_servo_axis_t *pobj, servo_axis_id_e id, const servo_axis_io_t *io)
{
    ...
    pobj->id = id;
    pobj->io = *io;
    pobj->dir = SERVO_AXIS_DIR_CW; /* TODO: 根据实际角度数据调整 */
...
void dev_servo_axis_load_default_param(dev_servo_axis_t *pobj)
{
    ...
    if (pobj->id == SERVO_AXIS_GD) {
        ...
        p->Kp_1_T1 = 2.0f;   // 1.5
        p->Ki_1_T1 = 3.5f;
        p->bound_T1 = 2.0f;  // bound 按误差大小做增益调度的分段 PI 跟踪环。
        p->Kp_2_T1 = 2.5f;   // 1.5
        p->Ki_2_T1 = 4.5f;
...
static void axis_load_default_config(dev_servo_axis_t *pobj)
{
    ...
    c->bmq_max_code = 16777216u; /* TODO : 根据实际编码器分辨率调整 */
    ...
    if (pobj->id == SERVO_AXIS_GD) {
        c->bmq_zero = 1024350;                               /* 零偏 188256 */
        c->bmq_coef_deg = SF_BMQ_COF_GD;               /* Bmq_cof_gd，每1个码对应的角度 */
        c->gyro_factor = 0.0000292035869364061814765f; /* TODO: 根据实际陀螺仪参数调整 */
        c->angle_up_limit = 82.0f;                     /* TODO: 俯仰轴角度限位 */
        c->angle_dn_limit = -12.0f;
        ...
        c->bmq_zero = 7297419;                        /* 零偏 */
        c->bmq_coef_deg = SF_BMQ_COF_FW;              /* TODO：根据实际编码器分辨率调整 */
        c->gyro_factor = 0.000029119908707921404318f; /* TODO: 根据实际陀螺仪参数调整 */
...
static void axis_update_in_place(dev_servo_axis_t *pobj)
{
    ...
    /* 只有连续超过 500 个周期都保持在误差范围内，才最终认为轴真正到位。*/
    if (st->inplace_dwell > SF_INPLACE_DWELL_TH) {
        st->in_place = true;
