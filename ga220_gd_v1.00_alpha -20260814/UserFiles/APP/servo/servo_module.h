...
typedef enum {
    ...
    SERVO_EXT_CMD_SELFCHECK = 0x0B   /**< 自检：无参数；方位保持零位，照准架依次转上/下限位后回零 */
} servo_ext_cmd_e;
...
/**
 * @brief 电动推杆锁：外部通信指令控制开/关（非阻塞，动作持续时间可配）。
 *        通信层收到锁指令后调用一次；本函数只给出方向并启动计时，
 *        推杆到时停止在 servo_module_handler() 内的 1ms 推进里自动完成。
 *
 * @param cmd        通信指令码：0x01=上锁（伸出） / 0x02=开锁（缩回）
 * @param action_ms  本次动作持续时间(ms)；传 0 使用模块默认值（2000ms）
